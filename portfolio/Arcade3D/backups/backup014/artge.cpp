...
void arc::arc3d::fillTriangle(arc::face* fcp)
{
	...
	*/
	...
	if(p2.x < p1.x) // trojkat ma 2 wierzcholekpo lewej stronie
	{
		...
		arc::findDelta(&delta,&p1,&p2,&p3,1,2);
		
		if(delta.err_code == arc::NON)
			dxl = delta.value;
		else return;
		
		
		arc::findDelta(&delta,&p1,&p2,&p3,1,3);
		...
		arc::findDelta(&delta,&p1,&p2,&p3,2,3);
		if(delta.err_code == arc::NON)
			dxl = delta.value;
		else return;
