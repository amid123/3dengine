...
arc::screen::screen()
{
	...
	srf = SDL_SetVideoMode(800, //szerokoœæ
				 600, //wysokoœæ
				 24, //iloœæ bitów koloru 8,16,32, dla 0 sam ustala.
...
arc::screen::screen(int w, int h ,int col_dpth)
{
	this->color_depth = col_dpth;
	this->w = w;
	this->h = h;
	// Inicjujemy SDL
	if(SDL_Init( SDL_INIT_VIDEO )<0)
	...
	srf = SDL_SetVideoMode(this->w, //szerokoœæ
				 this->h, //wysokoœæ
				 this->color_depth, //iloœæ bitów koloru 8,16,32, dla 0 sam ustala.
				 SDL_HWSURFACE | SDL_DOUBLEBUF); // flagi, o których szerzej poni¿ej.
														 ...
														 sleeper(30)
{
	...
	scr = new arc::screen(w,h,col_depth);
	...
	this->text = new arc::BMPSTRING(this->scr->returnSurfacePtr(),w,h);
	this->viewMode = EDGE_VIEW;
}

void arc::screen::putPixel8bit(int x, int y, colRGB* color)
{
...
void arc::screen::putPixel16bit(int x, int y, colRGB* color)
{
	// Jesli wykraczamy poza ekran - nie rysujemy
	if( (x >= 0) && (x < w) && (y >= 0) && (y < h) )
	{
...
void arc::screen::putPixel24bit(int x, int y, colRGB* color)
{
	// Jesli wykraczamy poza ekran - nie rysujemy
	if( (x >= 0) && (x < w) && (y >= 0) && (y < h) )
	{
...
void arc::screen::putPixel32bit(int x, int y, colRGB* color)
{
	// Jesli wykraczamy poza ekran - nie rysujemy
	if( (x >= 0) && (x < w) && (y >= 0) && (y < h) )
	{
...
{	// implementacja algorytmu rasteryzacji B.
	if( (x1 >= -w) && (x1 < w*2) && (y1 >= -h) && (y1 < h*2) )
		if( (x2 >= -w) && (x2 < w*2) && (y2 >= -h) && (y2 < h*2) )
		{
			 ...
			 this->scr->putPixel24bit(x, y,&col);
			 if (dx > dy)
			 {
				 ai = (dy - dx) * 2;
				 bi = dy * 2;
...
SDL_Surface * arc::screen::returnSurfacePtr()
{
	return this->srf;
}
		...
		for(unsigned int j = 0 ; j < this->mainScene->objects.size() ; j++)
		{
			//if(this->mainScene->objects[j].isVisible())
				for(unsigned int i = 0 ; i < this->mainScene->objects[j].vertexList.size ; i++)
				{
					...
					this->scr->putPixel24bit((int)(scr_x_zero + (this->mainScene->objects[j].vertexList.indexList[i]->scr.x)),
	...
	while(done)
	{
	    while(SDL_PollEvent(&event))
	    {
	        ...
	        if(event.type == SDL_KEYDOWN)
			{
				...
				{
					this->mainScene->objects[n].addRotate(arc::X,(float)DEG_TO_RAD(-1));
				}
				...
				{
					this->mainScene->objects[n].addRotate(arc::Y,(float)DEG_TO_RAD(-1));
				}
				...
				{
					this->mainScene->mainCamera->set(0,0,0,(float)DEG_TO_RAD(1),0,0);
					this->mainScene->mainCamera->build(YXZ);
				}
				if(event.key.keysym.sym == SDLK_a)
				{
					this->mainScene->mainCamera->set(0,0,0,0,(float)DEG_TO_RAD(-1),0);
					this->mainScene->mainCamera->build(YXZ);
				}
		...
		this->render();
		// poprawic implementacje funkcji stabilizujacej czas ramki :D
...
void arc::arc3d::render()
{
	...
	for(unsigned int i = 0 ; i < this->mainScene->objects.size() ; i++)
	{
		...
		for(unsigned int j = 0 ; j < this->mainScene->objects[i].vertexList.size ; j++)
		{
			...
			this->mainScene->objects[i].vertexList.indexList[j]->scr.x = dist * x_buf / z_buf;
			this->mainScene->objects[i].vertexList.indexList[j]->scr.y = dist* y_buf * aspect / z_buf;
		}
...
void arc::arc3d::drawHorizontalLine(int x1,int x2,int y)
{
	for( ; x1<x2 ; x1++ )
		this->scr->putPixel24bit(this->scr_x_zero + x1, this->scr_y_zero + y,&this->col);
}

void arc::arc3d::fillTriangle(arc::face* fcp)
{
	float dxr, xr;
	float dxl, xl;
	int y;

	arc::VECTOR2D p1;	// bufory dla wierzcholkow faca;
	arc::VECTOR2D p2;
	arc::VECTOR2D p3;
	arc::VECTOR2D swap_buffer;
	...
	// Sortujemy wierzcholki;

	if(p1.y > p2.y) SWAP(p1,p2,swap_buffer);
	if(p2.y > p3.y)
	{
		SWAP(p2,p3,swap_buffer);
		if(p1.y > p2.y) SWAP(p1,p2,swap_buffer);
	}

	//Obliczamy przyros x'ow :P tzw. delty

	/*
	** Tutaj wyliczamy delty dla trojkata z wertexem 2 po lewej stronie
	** pierwsza faza rasteryzacji tzn od wertexu 1 do 2
	** czyli pierwsza polowa trójkata :)
	*/

	if(p2.x < p1.x) // trojkat ma 2 wierzcholekpo lewej stronie
	{
		// pierwsza czesc trojkata
		dxl = arc::findDelta(&p1,&p2,&p3,1,2);
		dxr = arc::findDelta(&p1,&p2,&p3,1,3);
		...
		while(y<p2.y)
		{
			this->drawHorizontalLine((int)xl,(int)xr,y);
		...
		// teraz druga czesc trojkata
		dxl = arc::findDelta(&p1,&p2,&p3,2,3);
		while(y<p3.y)
		{
			this->drawHorizontalLine((int)xl,(int)xr,y);
	...
	else // trojkat ma 2 wierzcholek po prawej
	{
		// pierwsza czesc trojkata
		dxl = arc::findDelta(&p1,&p2,&p3,1,3);
		dxr = arc::findDelta(&p1,&p2,&p3,1,2);
		...
		// teraz druga czesc trojkata
		dxr = arc::findDelta(&p1,&p2,&p3,2,3);
		while(y<p3.y)
		{
			this->drawHorizontalLine((int)xl,(int)xr,y);
