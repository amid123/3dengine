	    ...
	    while(SDL_PollEvent(&event))
	    {
	        ...
	        if(event.type == SDL_KEYDOWN)
			{
				...
				if(event.key.keysym.sym == SDLK_w)
				{
					...
					this->mainScene->mainCamera->build();
				}
...
bool arc::arc3d::checkVisibleObject(unsigned int fov ,arc::object* obj)
{
	
	arc::VECTOR4D worldObjPos;
	arc::VECTOR4D camObjPos;

	arc::vct4d_init(&worldObjPos, -obj->pos_x(), -obj->pos_y(), -obj->pos_z(), 1);
	arc::mtx4x4mulvct4d(&this->mainScene->mainCamera->camMtx,&worldObjPos,&camObjPos);
