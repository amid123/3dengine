...
void arc::transformation::addTranslationGlobal(float x,float y,float z)
{
	for(unsigned int i = 0 ; i < this->vertexList.size ; i++)
	{
		this->vertexList.indexList[i]->global_x += x;
		this->vertexList.indexList[i]->global_y += y;
		this->vertexList.indexList[i]->global_z += z;
	}
}


arc::transformation::~transformation(){}
arc::object::object(char* filename) : transformation(filename), visible(true){return;}
